...
| **경로 표시줄 (breadcrumb)** | 상단 경로가 클릭 가능한 조각들로 표시됨. 조상 디렉터리를 클릭하면 그리로 이동. 선택한 파일/폴더를 조상 조각에 드래그해 드롭하면 그 상위 디렉터리로 이동 (같은 호스트) 또는 반대쪽에서 전송 — 바로 위 한 칸이 아니라 드롭한 조각 위치로. |
| **뒤로/앞으로 히스토리** | 패널별 `←` / `→` 버튼으로 그 패널의 이동 히스토리를 따라 이동. 히스토리는 원격 탭별 + 로컬 패널 각각 유지, 새로고침은 중복 항목을 만들지 않음. |
...
| **멀티 호스트 탭** | 활성 SSH 세션마다 탭 1개, 일치하는 preset이 있으면 그 이름으로 (없으면 주소로) 표시. `+`로 추가, `×`로 닫음 (세션 종료). 각 탭이 현재 경로와 트리 상태를 자기 안에서 유지. 이미 열려 있는 호스트로 접속하면 새 세션을 만들지 않고 그 탭으로 전환. |
| **R2R (원격↔원격)** | 상단바 토글. 누르면 좌측 패널이 활성 탭에 고정되고 우측 패널이 같은 세션으로 열림; 이후 탭 클릭은 **우측** 패널을 옮김 (탭바에 ◧ 좌, ◨ 우, ◫ 양쪽 표시). 다시 누르면 우측이 로컬로 돌아감. 서로 다른 호스트 사이의 드롭은 `/api/r2r`로 감 — v1은 로컬 릴레이 전략 (src → 로컬 임시 → dst); `sshpass` direct는 deferred ([로드맵](#로드맵) 참조). |
| **같은 세션 R2R** | 한 호스트를 양쪽에 — 같은 서버의 두 디렉터리를 나란히 보는 것이 R2R의 기본 상태. 이 경우 패널 간 드롭은 릴레이 없이 호스트 위에서 `cp -r`로 복사되어 네트워크를 타지 않고, 바뀐 디렉터리를 보고 있는 반대쪽 패널도 같이 갱신됨. |
