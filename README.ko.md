...
| **R2R (원격↔원격)** | 상단바 토글; 우측 패널이 로컬에서 원격으로 바뀜 (드롭다운으로 호스트 선택). 두 원격 사이의 드롭은 `/api/r2r`로 감. v1은 로컬 릴레이 전략 (src → 로컬 임시 → dst); `sshpass` direct는 deferred ([로드맵](#로드맵) 참조). |
| **같은 세션 R2R** | 드롭다운에 활성 탭 자신도 들어 있어서 한 호스트를 양쪽에 띄울 수 있음 — 같은 서버의 두 디렉터리를 나란히. 이 경우 패널 간 드롭은 릴레이 없이 호스트 위에서 `cp -r`로 복사되어 네트워크를 타지 않고, 바뀐 디렉터리를 보고 있는 반대쪽 패널도 같이 갱신됨. |
